...
<%
    ...
    if ((actual.getAttribute("Logueado") != null)&&(accessProducts!=0)) {
%>
    ...
    <body style="background: #1b1f22;">
            ...
            <header id="header">
                ...
                <div class="content">
                    <div class="inner">
                        ...
                        <form action="servlet_productos" method="POST" class="form-group" enctype="multipart/form-data" class="form-horizontal" role="form" name="formulario">
                            ...
                            <select name="box_Categoria" id="box_Categoria" class="form-control">
                                <%
                                    Categorias categoria = new Categorias();
                                    HashMap<String, String> comboCategorias = categoria.listaDeCategorias();
                                    out.println("<option value='0'>Seleccione</option>");
                                    for (String i : comboCategorias.keySet()) {
                                        out.println("<option value='" + i + "'>" + comboCategorias.get(i) + "</option>");
                                    }
                                %> 
                            </select><br>
                            ...
                            <input type="text" name="txt_precio" class="form-control" id="txt_precio" placeholder="Ejemplo:1200.00"  required><br>
                            ...
                            <br><br>
                            <input type="hidden" id="txt_idExistencias" name="txt_idExistencias">
        ...
        <script  type="text/javascript">
            $('#tbl_productos').on('click', 'tr td', function (evt) {
                ...
                var target, idproducto, idmarcas, idcategoria, idexistencia, descripcion, precio, cantidadInStock,
                ...
                idmarcas = target.parent().data('id_marca');
                idcategoria = target.parent().data('id_categoria');
                idexistencia = target.parent().data('id_existencia');
                imagen = target.parents().data('id_imagen');
                producto = target.parents("tr").find("td").eq(0).html();
                descripcion = target.parents("tr").find("td").eq(2).html();
                precio = target.parents("tr").find("td").eq(5).html();
                cantidadInStock = target.parents("tr").find("td").eq(6).html();
                inmediato = target.parents("tr").find("td").eq(7).html();

                $("#txt_id").val(idproducto);
                $("#box_marcas").val(idmarcas);
                $("#box_Categoria").val(idcategoria);
                $("#txt_producto").val(producto);
                $("#txt_descripcion").val(descripcion);
                $("#txt_precio").val(precio);
                $("#txt_exitencias").val(cantidadInStock);
                $("#imagenes").val(imagen);
                $("#txt_idExistencias").val(idexistencia);
                switch (inmediato) {
                    case "SI":
                    {
                        $("#drop_Entrega").val(1);
                ...
                $("#imagenPrevisualizacion").prop("src", "assets/img/" + imagen);
